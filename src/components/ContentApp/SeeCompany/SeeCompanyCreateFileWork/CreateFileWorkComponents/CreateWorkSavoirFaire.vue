<template>
  <div>
    ...
    <title-cards
      :subTitle="true"
      :title="'Validez les compétences de type savoir faire'"
      :themeColor="thColor"
    ></title-cards>
    ...
    <form class="cws_form" v-if="isTradeSelected" id="savoirSubmit" @submit.prevent="competenceSavoirFaireValidate">
      ...
      <div>
        ...
        <div class="cws_box_input_add_competence" v-if="addSavoirFaire === true">
          <div>
            <input-sample
...
export default {
  ...
  data() {
    return {
      //watch new input for competence
      addSavoirFaire: false,
      //object for create new competence savoir
      newCompetenceSavoirFaire: { typeCompetence: "SavoirFaire", libelle: null },
    };
  },
  computed: {
    ...
    competenceSavoirFaire: {
      get() {
        return this.$store.state.ApiRom.competenceSavoirFaire;
      },
      set(value) {
        this.$store.commit("ApiRom/selectedCompetenceSavoirFaire", value);
      },
    },
  ...
  methods: {
    ...
    seeInputCompetenceSavoirFaire() {
      this.addSavoirFaire = !this.addSavoirFaire;
    },

    //for hidd new input
    hiddInputCompetenceSavoirFaire() {
      this.addSavoirFaire = !this.addSavoirFaire;
    },
    ...
    addCompetenceSavoirFaire() {
      if (this.newCompetenceSavoirFaire.libelle != null) {
        this.competenceBasic = this.newCompetenceSavoirFaire;
        this.addSavoirFaire = !this.addSavoirFaire;
      }
